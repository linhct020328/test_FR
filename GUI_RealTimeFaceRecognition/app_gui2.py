...
from tkinter import font as tkfont, RIGHT, BOTH
...
class MainUI(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (LoginPage, StartPage, PageOne, PageTwo, PageThree, PageFour, PageFive):
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.button3 = tk.Button(self.label_frame, text="Door status", fg="#ffffff", bg="#3366FF", font='Helvetica 12 bold',
...
class PageFour(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.button2.grid(row=2, column=0, sticky="ew", ipadx=10, ipady=4, padx=40, pady=10)
...
class PageFive(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        self.lable = tk.Label(self, text="DOOR OPEN/CLOSE STATUS OF INTELLIGENT ANTI-THEFT SYSTEM", fg="#3366FF",
                              font=self.controller.title_font)
        self.lable.grid(row=0, column=0, sticky="ew", padx=40, pady=10)
        self.label_frame = tk.LabelFrame(self)
        self.label_frame.grid(row=1, column=0, sticky="ew", padx=40, pady=20)

        self.label_frame_list = tk.LabelFrame(self.label_frame)
        self.label_frame_list.grid(row=0, column=0, sticky="ew", columnspan=3, padx=40, pady=20)

        self.yScroll = tk.Scrollbar(self.label_frame_list, orient=tk.VERTICAL)
        self.yScroll.grid(row=0, column=1, sticky='ns')
        self.xScroll = tk.Scrollbar(self.label_frame_list, orient=tk.HORIZONTAL)
        self.xScroll.grid(row=1, column=0, sticky='ew')
        self.listStates = tk.Listbox(self.label_frame_list, height=13, fg="#3366FF", font='Helvetica 12 bold', xscrollcommand=self.xScroll.set, yscrollcommand=self.yScroll.set)
        self.listStates.grid(row=0, column=0, sticky='nsew', ipadx=200)
        self.xScroll['command'] = self.listStates.xview
        self.yScroll['command'] = self.listStates.yview
        ...
        for state in list_states:
            if state == 'None':
                continue
            self.listStates.insert(tk.END, state)
        f.close()

        self.buttonRefresh = tk.Button(self.label_frame, text="Refresh", command=self.refreshState, fg="#ffffff", bg="#3366FF", font='Helvetica 12 bold')
        self.buttonReset = tk.Button(self.label_frame, text="Reset", command=self.resetState, fg="#ffffff", bg="#3366FF", font='Helvetica 12 bold')
        self.buttoncanc = tk.Button(self.label_frame, text="Cancel", command=lambda: controller.show_frame("StartPage"), font='Helvetica 12 bold', bg="#ffffff", fg="#DD0000")

        self.buttonRefresh.grid(row=1, column=0, sticky="ew", ipadx=10, ipady=4, padx=40, pady=10)
        self.buttonReset.grid(row=1, column=1, sticky="ew", ipadx=10, ipady=4, padx=40, pady=10)
        self.buttoncanc.grid(row=1, column=2, sticky="ew", ipadx=10, ipady=4, padx=40, pady=10)

    def refreshState(self):
        self.listStates.delete(0, tk.END)
        list_states = []
        f = open("listStates/stateslist.txt", "r")
        x = f.read()
        z = x.rstrip().split("\n")
    ...
    def resetState(self):
        if messagebox.askokcancel("Quit", "Are you sure?"):
            ...
            for i in z:
                ...
                list_states.append(smallList)
            f.close()
            f = open("listStates/stateslist.txt", "w")
            f.write('None')
            f.close()
            self.listStates.delete(0, tk.END)

            self.controller.show_frame("PageFive")
            self.controller.frames["PageFive"].refreshState()
