...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  int inputLen = sizeof(mesages);
  int decodedLen = base64_dec_len(mesages, inputLen);
  char decoded[decodedLen];
  
  base64_decode(decoded, mesages, inputLen);
  Serial.println(decoded);
  
  Serial.println();
  if (strcmp(decoded,CMD_OPEN) == 0 && directionState == 0) {
    directionState = 1;
    openDoor();
  } 
  else if (strcmp(decoded,CMD_CLOSE) == 0 && directionState == 1) {
    directionState = 0;   
    closeDoor();
  }
}
