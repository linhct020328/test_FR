...
void closeDoor(){
  String msgOpen = "closeDoor";
  for (pos = 0; pos <= 135; pos += 1) {
    myservo.write(pos);
    delay(15);
  }

  String encdataOpen = encrypt(msgOpen);
  
  client.publish(topic, (char*)encdataOpen.c_str()); 
  client.subscribe(topic);
}

void openDoor(){
  String msgClose = "openDoor";
  for (pos = 135; pos >= 0; pos -= 1) {
    myservo.write(pos);
    delay(15);
  }
  String encdataClose = encrypt(msgClose);
  
  client.publish(topic, (char*)encdataClose.c_str()); 
  client.subscribe(topic);
}
